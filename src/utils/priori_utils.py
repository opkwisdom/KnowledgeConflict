...
def get_pj_prompt(sample, use_ra: bool = False):
    paras = ""
    prompt = prompt_dict["prior"]['none']
    
    # Retrieval-Augmented(RA) 설정인 경우 (top-1 문서 사용)
    if use_ra:
        if 'ctxs' in sample and len(sample['ctxs']) > 0:
            top_ctx = sample['ctxs'][0]
            # 논문의 권장 포맷: Title + Content [cite: 531]
            paras = f"Title: {top_ctx['title']}\nContent: {top_ctx['text']}"
        
        prompt = prompt_dict["prior"]['ra']
    
    # Chat 모델인 경우 꼬리 질문 생략 (llm.py 로직 대응)
    tail = ""
    prediction = ""
