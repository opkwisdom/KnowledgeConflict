...
ret_path = "/workspaces/kvzip_nlplab/DISCA/results/rag/hotpotqa-w/prompt=base/20260428_145502/inference_results.json"
...
def extract_metrics(ret_results, gen_results):
    ...
    print(f"Retrieval better than Generation: {len(ret_over_gen)}")
    print(f"Generation better than Retrieval: {len(gen_over_ret)}")
    print(f"Both correct: {len(overlap)}")
    print(f"Both incorrect: {len(fail)}")
