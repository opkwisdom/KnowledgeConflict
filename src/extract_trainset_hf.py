...
from judge_model import OpenAIJudger, HfLLMJudger, JudgeOutput, CtxsRelevance
from utils import (
...
def judge_and_save(
    config: DictConfig,
    llm_judger: Union[OpenAIJudger, HfLLMJudger],
    data: List[QAExample],
    save_interval: int = 1000,
) -> None:
    logger = logging.getLogger(__name__)
    setup_seed(config.seed)
    # indices = np.random.randint(0, len(data), NUM_SAMPLES)
    # sampled = [data[i] for i in indices]

    match = re.search(r'(\d+B)', llm_judger.llm_model_name, re.IGNORECASE)
    model_size = match.group(0).upper() if match else "unknown"
    file_name = f"full_train_data_{model_size}.json" \
        if "reasoning" not in config.llm_judger.prompt_name \
        else f"full_train_data_{model_size}_reasoning.json"
    ...
    # filtered_samples = sampled[20000:]
    # batches = [filtered_samples[i:i + config.data.batch_size] for i in range(0, len(filtered_samples), config.data.batch_size)]
    ...
    for i, batch in tqdm(enumerate(batches), desc="Judge Data", total=len(batches)):
        ...
        # LLM Judging
        judge_output_list: List[CtxsRelevance] = llm_judger.batch_judge(
            batch_queries,
            batch_answers,
            batch_ctxs,
        )
        if not judge_output_list:
            logger.warning(f"Batch {i} judging failed. Skipping this batch.")
            continue
    ...
    if llm_judger.processed_count > 0:
...
def main():
    ...
    # Initialize judger
    llm_judger = HfLLMJudger(config.llm_judger)
    logger.info(f"LLM Judger ({llm_judger.llm_model_name}) initialized.")
