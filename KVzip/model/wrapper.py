...
class ModelKVzip():
    def __init__(self, model_name: str, kv_type: str = "evict", gen_kwargs: dict = None, prompt: str = "", logger=None, attn_type = None):
        self.model, self.tokenizer = load_model(model_name, attn_type=attn_type)
    ...
    def self_task(
        ...
        # Q + A_internal + Prompt + SYS_POST
        if a_ids is not None:
            base_prefix = torch.cat([q_ids, a_ids], dim=1)  # Q + A_internal
            for i, c_ids in enumerate(chunked_inputs):
                prompt_ids = self.encode(self.prompt)
                current_q_ids = torch.cat([base_prefix, prompt_ids], dim=1)
                full_input = torch.cat([current_q_ids, self.postfix_ids, a_ids], dim=1) # Repeat query (Q + A_internal + Prompt + SYS_POST) + A_internal (Teacher forcing)
                input_ids.append((c_ids, full_input))  # (context chunk, full input)
        else:
            # Pseudo-passage case
            for i, pseudo_c_ids in enumerate(chunked_inputs):
                prompt_ids = self.encode(self.prompt)
                full_input = torch.cat([prompt_ids, self.postfix_ids, q_ids], dim=1)    # Repeat prompt + SYS_POST + Context (Teacher forcing)
                input_ids.append((pseudo_c_ids, full_input))    # (pseudo-passage chunk, full input)
